...
export const getTweetController = async (req: Request<TweetParam, any, any, any>, res: Response) => {
...
export const getRandomNewFeedsController = async (req: Request<ParamsDictionary, any, any>, res: Response) => {
  const user_id = req.decoded_authorization?.user_id as string;
  const result = await tweetsService.getRandomNewFeeds(user_id);
  return res.json({
    message: 'Get New Feeds Successfully',
    result: {
      tweets: result.tweets
